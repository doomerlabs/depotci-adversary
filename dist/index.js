...
var BrokerReviewModel = class {
  ...
  async #reviewOnce(normalized, signal) {
    ...
    if (!response.ok) {
      const failure2 = decoded;
      throw new ModelReviewError(failure2.error?.message ?? `Model broker returned HTTP ${response.status}.`, {
        code: failure2.error?.code ?? "model_review_failed",
        retryable: failure2.error?.retryable ?? response.status >= 500
      });
    }
...
var LONG_STEP_LABEL = /\b(?:build|tests?|integration|e2e|scan|audit|publish|release|deploy|coverage|race|docker|buildx|migration)\b/i;
var LONG_RUN_COMMAND = /\b(?:docker\s+(?:build|push)|buildx|cargo\s+(?:build|test)|pytest|go\s+test|npm\s+(?:test|publish)|pnpm\s+(?:test|publish)|yarn\s+(?:test|publish)|make\s+(?:test|build)|kubectl|helm\s+upgrade)\b/i;
var RELEASE_PATTERN = /\b(?:publish|release|deploy|production|push(?:es|ing)?\s+(?:an?\s+)?(?:image|artifact|package)|npm publish|cargo publish|twine upload|docker push|gh release|kubectl|helm upgrade)\b/i;
...
function isLongRunningJob(job) {
  ...
  return job.steps.some((step) => LONG_STEP_LABEL.test(`${step.name} ${step.uses ?? ""}`) || LONG_RUN_COMMAND.test(step.run ?? "") || isBuildAction(step.uses));
}
...
function isExpensiveWorkflow(workflow) {
  return workflow.jobs.some((job) => isLongRunningJob(job));
}
function allJobReferences(value) {
  const text2 = expressionText(value);
  return [...text2.matchAll(/\bneeds\.([A-Za-z0-9_-]+)\b/g)].map((match) => match[1]).filter((value2) => value2 !== void 0);
}
function secretNames(value) {
  const text2 = expressionText(value);
  return [...new Set([...text2.matchAll(/\bsecrets\.([A-Za-z0-9_]+)\b/g)].map((match) => match[1]).filter((name) => name !== void 0))].sort();
}
...
function permissionWrites(value) {
  if (typeof value === "string") {
  ...
  return Object.entries(value).filter(([, permission]) => typeof permission === "string" && permission.toLowerCase() === "write").map(([scope]) => scope).sort();
}
function hasCacheUse(job) {
...
function conditionAllowsPullRequest(job) {
...
function isCheckoutOfPullRequestHead(step) {
...
function executesRepositoryCodeAfterCheckout(job, checkoutIndex) {
  return job.steps.find((step) => step.index > checkoutIndex && (step.run !== void 0 || isBuildAction(step.uses)));
}
function isBuildAction(uses) {
  return /(?:depot|docker)\/build-push-action@/i.test(uses ?? "");
}
function isPullRequestWorkflow(workflow) {
...
var DELIVERY_ACTION = /\b(?:auth|attest|build-push|credential|deploy|login|publish|release|sign)\b/i;
var PINNED_ACTION_CONTRACTS = [{
...
function analyzeActions(workflow) {
  const detections = [];
  for (const job of workflow.jobs) {
    if (job.uses !== void 0) {
      const parsed = parseExternalReference(job.uses);
      if (parsed !== void 0 && !FULL_COMMIT_SHA.test(parsed.ref)) {
...
function parseExternalReference(uses) {
...
function unsupportedActionInputs(workflow, job, step, action, ref) {
  const contract = PINNED_ACTION_CONTRACTS.find((candidate) => candidate.action.toLowerCase() === action.toLowerCase() && candidate.ref.toLowerCase() === ref.toLowerCase());
  if (contract === void 0) {
    return [];
  }
  const supported = new Set(contract.inputs.map((input) => input.toLowerCase()));
  return Object.entries(step.inputLocations).filter(([input]) => !supported.has(input.toLowerCase())).sort(([left], [right]) => left.localeCompare(right)).map(([input, location]) => ({
    ...
    locality: { kind: "direct", anchors: [location.line] }
  }));
}
function unpinnedDetection(workflow, job, step, subject, location, ownerRepository, ref) {
  ...
  return {
    ...
    severity: authority.tier === "privileged-delivery" || authority.tier === "privileged" ? Severity.High : authority.tier === "delivery" ? Severity.Medium : Severity.Low,
    file: workflow.path,
    line: location.line,
    snippet: location.snippet,
    ...
    ...authority.tier === "routine" ? { locality: { kind: "direct", anchors: [location.line] } } : {}
  };
}
function actionAuthority(workflow, job, step, ownerRepository) {
  ...
  const releaseContext = DELIVERY_ACTION.test(`${workflow.name} ${workflow.path}`) || isReleaseJob(job) || DELIVERY_ACTION.test(ownerRepository) || step !== void 0 && DELIVERY_ACTION.test(stepText(step));
  const reasons = [
    ...writePermissions.length > 0 ? [`This action executes with repository write permissions (${writePermissions.join(", ")}).`] : [],
    ...credentials.length > 0 ? [`This action receives publishing or deployment credentials (${credentials.join(", ")}).`] : [],
    ...releaseContext ? ["This action runs in a release, publishing, signing, or deployment path."] : []
  ];
  const tier = writePermissions.length > 0 || credentials.length > 0 ? releaseContext ? "privileged-delivery" : "privileged" : releaseContext ? "delivery" : "routine";
...
function mutableRunInputs(workflow, job, step, run, line, snippet) {
  if (run === void 0) {
    return [];
  }
  const executed = executableShellLines(run);
  const reasons = [];
  if (executed.some((line2) => isExecutedCommand(line2, "(?:curl|wget)") && /(?:raw\.githubusercontent\.com|github\.com\/[^\s]+\/raw\/)(?:[^\s]+\/)?(?:main|master|HEAD)\b/i.test(line2))) {
    reasons.push("script is downloaded from a mutable branch");
  }
  if (executed.some((line2) => isExecutedCommand(line2, "(?:curl|wget)") && /\|\s*(?:sudo\s+)?(?:ba)?sh\b/i.test(line2))) {
    reasons.push("downloaded content is executed directly without an integrity check");
  }
  if (executed.some((line2) => isExecutedCommand(line2, "docker") && /\bdocker\s+(?:pull|run)\s+[^\s]+:latest\b/i.test(line2))) {
...
function executableShellLines(run) {
  const lines = run.split(/\r?\n/);
  const physicalLines = [];
  let heredocTerminator;
  for (const line of lines) {
    if (heredocTerminator !== void 0) {
      if (line.trim().replace(/^\t+/, "") === heredocTerminator) {
        heredocTerminator = void 0;
      }
      continue;
    }
    const trimmed = line.trim();
...
function shellContinuation(line) {
  const trailing = line.match(/(\\+)\s*$/);
  if (trailing === null || trailing[1].length % 2 === 0 || trailing.index === void 0) {
...
function isExecutedCommand(line, command) {
  return new RegExp(`(?:^|&&\\s*|\\|\\|\\s*|[;|]\\s*|\\$\\(\\s*|\\b(?:then|do)\\s+)(?:sudo\\s+)?${command}\\b`, "i").test(line);
}

// src/rules/cache.ts
function analyzeCaching(workflow, repository) {
  return [
...
function unstableCacheKeys(workflow) {
  const detections = [];
  for (const job of workflow.jobs) {
    for (const step of job.steps.filter(isCacheStep)) {
      const key = typeof step.with.key === "string" ? step.with.key : void 0;
      if (key === void 0) {
        continue;
      }
...
function missingDependencyCaches(workflow, repository) {
  const detections = [];
  for (const job of workflow.jobs) {
    if (hasCacheUse(job)) {
      continue;
...
function cacheLifecycle(workflow) {
  const detections = [];
  for (const job of workflow.jobs) {
    ...
    for (const restore of restores) {
      const restoreKey = typeof restore.with.key === "string" ? restore.with.key : void 0;
      const matchingSave = saves.find((save) => save.index > restore.index && keysCorrespond(restoreKey, typeof save.with.key === "string" ? save.with.key : void 0));
      if (matchingSave !== void 0) {
        continue;
      }
      detections.push({
...
function dockerfileCacheOrder(workflow, repository) {
...
function cacheKeyWeaknesses(job, step, key) {
  const lower = key.toLowerCase();
  const reasons = [];
  const manager = packageManagerInstall(job);
  const hasFallbackKeys = typeof step.with["restore-keys"] === "string" && step.with["restore-keys"].trim().length > 0;
  if (/github\.sha|github\.event\.after|commit(?:[_-]?sha)?/.test(lower) && (manager !== void 0 || hasFallbackKeys)) {
    reasons.push("the full commit identity prevents reuse across commits");
  }
  if (manager !== void 0 && !/hashfiles\s*\(/i.test(key)) {
...
function isCacheStep(step) {
  return /actions\/cache(?:\/(?:restore|save))?@/i.test(step.uses ?? "");
}
function packageManagerInstall(job) {
...
function installPattern(manager) {
  return {
...
function lockfilesForManager(lockfiles, manager) {
  const pattern = {
...
function keysCorrespond(restore, save) {
  return restore !== void 0 && save !== void 0 && restore.trim() === save.trim();
}
function broadCopyBeforeDependencyInstall(source) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (broadCopy !== void 0 && /^RUN\s+.*\b(?:npm\s+(?:ci|install)|pnpm\s+install|yarn\s+install|go\s+mod\s+download|cargo\s+(?:fetch|build)|pip\s+install)\b/i.test(normalized)) {
...
var RULES = [
  ...
  {
    id: "depotci.action.unsupported-input",
    category: "correctness",
    severity: Severity.Medium,
    confidence: Confidence.High,
    title: { singular: "Pinned action receives an unsupported input", plural: "Pinned actions receive unsupported inputs" },
    summary: (count, subjects) => `${count} explicitly configured action input${count === 1 ? " is" : "s are"} absent from the verified pinned action contract (${formatSubjects(subjects)}).`,
    whyItMatters: "GitHub Actions ignore unsupported inputs after warning, so a workflow can appear to enable a lock, permission, cache mode, or other control that the action never receives.",
    impact: "The intended action behavior is not configured, while maintainers may incorrectly rely on a control that has no effect.",
    recommendation: "Remove the unsupported input or update to an exact action revision whose action metadata documents the required input and semantics; do not infer inputs from a different action or cache implementation.",
    complexity: "small",
    tags: ["actions", "correctness", "contract"]
  },
  {
  ...
  },
  {
    ...
    summary: (count) => `${count} workflow step${count === 1 ? "" : "s"} interpolate untrusted github.event fields into run: shell.`,
    whyItMatters: "Issue titles, comment bodies, and similar fields can contain shell metacharacters that break out of unquoted expansions.",
    impact: "Attackers can inject commands into privileged CI and steal secrets or modify artifacts.",
    recommendation: "Pass untrusted values through env: and quote them in shell, or avoid shell interpolation entirely.",
    complexity: "small",
  ...
  },
  {
    ...
    summary: (count) => `${count} job${count === 1 ? "" : "s"} run on self-hosted runners for pull_request workflows.`,
    whyItMatters: "Public PR code on persistent self-hosted runners exposes the host and any credentials available to that machine.",
    impact: "Attackers can persist on the runner, steal secrets, or pivot into the network.",
    recommendation: "Use GitHub-hosted or ephemeral runners for untrusted PR code; keep secrets off long-lived self-hosted fleets.",
    complexity: "small",
  ...
  },
  {
    ...
    summary: (count) => `${count} job${count === 1 ? "" : "s"} inject secrets at job scope across more than one step.`,
...
var RULE_MAP = new Map(RULES.map((rule) => [rule.id, rule]));
function registerDepotRules(app) {
...
function synthesizeUnpinnedActions(observations) {
  ...
  const affected = [...actionCounts.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([action, count]) => `${action} (${count})`);
  ...
  if (tier === "privileged-delivery" || tier === "privileged") {
    ...
    return {
      title: privilegedDelivery ? "Release workflows execute mutable actions with production authority" : "Mutable actions execute with elevated workflow authority",
      category: "supply-chain",
...
${affected.map((item) => `- ${item}`).join("\n")}`,
      ...
      recommendation: privilegedDelivery ? "Pin the release-critical actions to full commit SHAs first, then keep those pins current with an automated dependency update workflow." : "Pin actions that execute with elevated permissions or credentials to full commit SHAs, then keep those pins current with reviewed automation.",
      remediation: { complexity: "small" },
      tags: privilegedDelivery ? ["actions", "release", "supply-chain", "runtime"] : ["actions", "permissions", "supply-chain", "runtime"]
    };
  }
  if (tier === "delivery") {
    return {
...
function observationData(observation) {
  return typeof observation.evidence === "object" && observation.evidence !== null && !Array.isArray(observation.evidence) ? observation.evidence : {};
}
...
function highestConfidence(observations, fallback) {
  const ranks = { low: 0, medium: 1, high: 2 };
  let highest = Confidence.Low;
  for (const observation of observations) {
...
function missingTimeouts(workflow) {
  return workflow.jobs.filter((job) => job.timeoutMinutes === void 0 && isLongRunningJob(job)).map((job) => ({
...
function dependencyStructure(workflow) {
  const detections = [];
  ...
  for (const job of workflow.jobs) {
    ...
    for (const step of job.steps.filter((candidate) => /actions\/download-artifact@/i.test(candidate.uses ?? ""))) {
      const artifactName = typeof step.with.name === "string" ? step.with.name : void 0;
...
function failureMasking(workflow) {
  const detections = [];
  for (const job of workflow.jobs) {
    for (const step of job.steps) {
...
function releaseGates(workflow) {
  const detections = [];
  for (const job of workflow.jobs.filter(isReleaseJob)) {
    ...
    if (!hasValidationAncestor && !hasEarlierValidation) {
      const step = releaseIndex >= 0 ? job.steps[releaseIndex] : void 0;
      detections.push({
...
function missingConcurrency(workflow) {
  if (!isPullRequestWorkflow(workflow) || !isExpensiveWorkflow(workflow) || workflow.concurrency !== void 0) {
    return [];
  }
  return [{
...
function redundantWork(workflow) {
...
function dependencyDetection(workflow, job, label, data) {
  return {
    ruleId: "depotci.job.dependency-structure",
    subject: job.id,
    groupKey: `depotci.job.dependency-structure:${workflow.path}`,
...
function findArtifactProducers(workflow) {
  const result = /* @__PURE__ */ new Map();
  for (const job of workflow.jobs) {
    for (const step of job.steps.filter((candidate) => /actions\/upload-artifact@/i.test(candidate.uses ?? ""))) {
      const name = typeof step.with.name === "string" ? step.with.name : void 0;
      if (name !== void 0) {
...
function longRunningSignals(job) {
  return job.steps.filter(isImportantStep).map((step) => step.name).slice(0, 5);
}
...
function isValidationBoundary(job) {
...
function normalizeExpensiveCommand(run) {
  if (run === void 0) {
    return void 0;
  }
...
function detectCiSecurityIssues(file, source) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (hasPullRequestTarget && /github\.event\.pull_request\.head\.(?:sha|ref)/i.test(line)) {
      hits.push({
    ...
    if (/^\s*(?:-\s*)?run:/.test(line) || i > 0 && isContinuedRunBlock(lines, i)) {
      const inj = /\$\{\{\s*github\.event\.(?:issue|comment|discussion|pull_request|review|review_comment|head_commit)\.[^}]+\}\}/i.exec(
        line
      );
      if (inj) {
        hits.push({
...
function isContinuedRunBlock(lines, index) {
...
function detectJobLevelSecrets(file, lines) {
  const hits = [];
  let inJob = false;
  let jobLine = 0;
  let jobSnippet = "";
...
// src/rules/security.ts
function analyzeSecurity(workflow, repoPath = "") {
  return [
...
function sharedCoreDetections(workflow, repoPath) {
  let source = "";
  try {
    const absolute = repoPath ? join4(repoPath, workflow.path) : workflow.path;
  ...
  for (const hit of hits) {
    ...
    if (!mapped) continue;
    if (mapped === "depotci.action.unpinned" || mapped === "depotci.permissions.broad" || mapped === "depotci.pull-request.untrusted-code" || mapped === "depotci.secret.scope-broad") {
      continue;
    }
    const ruleId = mapped;
    out.push({
      ...
      ...hit.key === "script-injection" || hit.key === "runs-on-expression" ? { locality: { kind: "direct", anchors: [hit.line] } } : {}
    });
  }
  return out;
}
function secretScope(workflow) {
  const detections = [];
...
function untrustedPullRequestExecution(workflow) {
...
function broadPermissions(workflow) {
  const detections = [];
...
function isNarrowPullRequestFeedbackJob(workflow, job, writes) {
  ...
  return job.steps.every((step) => step.run === void 0 && isImmutableOrLocalAction(step.uses));
}
function isImmutableOrLocalAction(uses) {
  if (uses === void 0 || uses.startsWith("./")) {
    return true;
  }
  const separator = uses.lastIndexOf("@");
  return separator > 0 && /^[0-9a-f]{40}$/i.test(uses.slice(separator + 1));
}
function unsafeSecretUses(step) {
  const run = step.run ?? "";
...
function reportPrimaryOpportunities(ctx, detections) {
...
function primaryOpportunities(detections) {
  const byRemediation = /* @__PURE__ */ new Map();
  for (const detection of detections) {
    ...
    if (current === void 0 || SEVERITY_RANK[severity] > SEVERITY_RANK[current.severity] || severity === current.severity && impact > current.impact) {
      byRemediation.set(key, { opportunity, severity, impact });
    }
  }
  return [...byRemediation.entries()].sort(([leftKey, left], [rightKey, right]) => SEVERITY_RANK[right.severity] - SEVERITY_RANK[left.severity] || right.impact - left.impact || leftKey.localeCompare(rightKey)).slice(0, 5).map(([, value]) => value.opportunity);
}
function materialAssessment(detections) {
  ...
  if (privilegedActions.length > 0) {
    const permissions = uniqueStrings2(privilegedActions.flatMap((detection) => stringArray(detection.data.writePermissions)));
    const credentials = uniqueStrings2(privilegedActions.flatMap((detection) => stringArray(detection.data.secretNames)));
    const authority = [
  ...
  if (elevatedActions.length > 0) {
    const permissions = uniqueStrings2(elevatedActions.flatMap((detection) => stringArray(detection.data.writePermissions)));
    const credentials = uniqueStrings2(elevatedActions.flatMap((detection) => stringArray(detection.data.secretNames)));
    const authority = [
      ...permissions.length > 0 ? [`repository write permissions (${permissions.join(", ")})`] : [],
...
function opportunityFor(detection) {
...
function operationalPriority(detection) {
  if (detection.ruleId === "depotci.action.unpinned") {
    return detection.data.authorityTier === "privileged-delivery" ? 90 : detection.data.authorityTier === "privileged" ? 85 : detection.data.authorityTier === "delivery" ? 65 : 20;
  }
  const priorities = {
...
async function analyzeRepository(ctx, discovery, repository) {
  const detections = discovery.failures.map((failure2) => ({
    ruleId: "depotci.workflow.parse-error",
    subject: failure2.path,
    groupKey: `depotci.workflow.parse-error:${failure2.path}`,
    file: failure2.path,
    line: failure2.line,
    snippet: failure2.snippet,
    label: `${failure2.path}:${failure2.line}${failure2.column === void 0 ? "" : `:${failure2.column}`} could not be parsed`,
...
async function changeLocalDetections(ctx, detections) {
  ...
  const changedFiles = new Set(ctx.change.changedFiles.map(normalizePath4));
  const directPaths = [...new Set(detections.filter((detection) => detection.locality?.kind === "direct").map((detection) => normalizePath4(detection.file)).filter((path) => changedFiles.has(path)))];
  ...
  return detections.filter((detection) => {
    const path = normalizePath4(detection.file);
...
async function changedLineNumbers(ctx, path) {
  ...
  if (head !== void 0 && !ctx.change?.worktree) {
    args.push(head);
  }
  args.push("--", path);
  try {
...
function reportPositives(ctx, workflows, detections) {
  for (const workflow of workflows) {
    ...
    if (longJobs.length > 0 && longJobs.every((job) => job.timeoutMinutes !== void 0)) {
      ctx.review.positive({
        key: `depotci.jobs.timeouts:${workflow.path}`,
        summary: longJobs.length === 1 ? `Sets an explicit timeout on the long-running job in ${workflow.path}.` : `Sets explicit timeouts on all ${longJobs.length} long-running jobs in ${workflow.path}.`,
        evidence: longJobs.slice(0, 5).map((job) => ({ file: workflow.path, line: job.location.line, label: `${job.id}: ${job.timeoutMinutes} minutes` }))
      });
    }
    const releaseValidation = workflow.jobs.filter(isReleaseJob).flatMap((job) => job.steps.filter(isReleaseVersionValidation).map((step) => ({ job: job.id, step })));
...
function reportReview(ctx, discovery, detections) {
  ...
  const risk = highestRisk2(detections);
  if (risk === "none") {
  ...
  ctx.review.opinion({
    ship: false,
    summary: primary === void 0 ? "I would address the material workflow findings before production use." : `I would ${trimTrailingPeriod(lowercaseFirst2(primary))} before relying on these workflows for production delivery.`
  });
}
function highestRisk2(detections) {
...
function isExternalAction(value) {
  return value !== void 0 && !value.startsWith("./") && !value.startsWith("../") && !value.startsWith("docker://") && value.includes("@");
}
function isEffectiveCancellation(value) {
  return isRecord3(value) && value["cancel-in-progress"] === true && value.group !== void 0;
}
function isReleaseVersionValidation(step) {
...
function lowercaseFirst2(value) {
  return `${value.slice(0, 1).toLowerCase()}${value.slice(1)}`;
}
...
async function discoverDepotWorkflows(repoPath) {
...
function isLikelyWorkflowSource(path, source) {
  const filename = path.split("/").pop()?.toLowerCase() ?? "";
  return path.toLowerCase().startsWith(".depot/workflows/") || filename === "depot.yml" || filename === "depot.yaml" || /^depot-.+\.ya?ml$/.test(filename) || /^\s*(?:on|jobs)\s*:/m.test(source);
}
async function walkRepository(root, limit = MAX_DISCOVERY_FILES) {
  const files = [];
  async function visit(relativeDirectory) {
    if (files.length >= limit) {
      return;
    }
    const directory = join6(root, relativeDirectory);
    ...
    for (const entry of entries) {
      ...
      const relativePath = relativeDirectory.length === 0 ? entry.name : join6(relativeDirectory, entry.name);
      if (entry.isDirectory()) {
        if (!SKIPPED_DIRECTORIES.has(entry.name)) {
          await visit(relativePath);
        }
      } else if (entry.isFile()) {
        files.push(toPosixPath2(relativePath));
      }
    }
  }
  await visit("");
...
function isDepotWorkflowCandidate(path) {
